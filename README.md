...
```
 ...
 It loads the breastCancer dataset into the code. The diabetes dataset is a dataset of patients with diabetes. It consists of 8 features and 1 label.
```ruby
...
```
 ...
 It loads the breast cancer dataset into the code. The breast cancer dataset is a dataset of patients with diabetes. It consists of 8 features and 1 label.
```ruby
data = pd.read_csv('breastcancer.csv', usecols=[0, 1])  # Read only the first and second columns (id and diagnosis)
dataset = data.values
```
...
[Youtube link:] (https://youtu.be/qz96yRXoRrw)
